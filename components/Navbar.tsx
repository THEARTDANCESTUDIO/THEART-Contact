    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'bg-white/95 backdrop-blur-md py-4 shadow-sm' : 'bg-transparent py-7'}`}>
        <div className="max-w-[1440px] mx-auto px-6 md:px-12 flex justify-between items-center">
          <a href={homeLink} className="group text-2xl font-black tracking-tighter uppercase flex flex-col leading-none">
          ...
          <div className="hidden lg:flex items-center space-x-12 text-[12px] font-bold uppercase tracking-[0.15em]">
            {currentMenu.map((item) => (
              <a key={item.name} href={item.link} className="relative group overflow-hidden">
                <span className="block group-hover:-translate-y-full transition-transform duration-500 text-black">{item.name}</span>
                <span className="absolute top-full left-0 block group-hover:-translate-y-full transition-transform duration-500 text-gray-400">{item.name}</span>
              </a>
            ))}
          </div>

          <div className="hidden lg:flex items-center space-x-8">
            <button onClick={() => setIsGlobalOpen(true)} className="border-l border-gray-200 pl-6 flex items-center space-x-2 cursor-pointer hover:opacity-50 transition-all duration-300">
        ...
        <div className={`fixed inset-0 bg-white z-40 transition-transform duration-700 ease-in-out lg:hidden pt-24 px-8 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="flex flex-col space-y-8 mt-12">
            {currentMenu.map((item) => (
              <a key={item.name} href={item.link} className="text-4xl font-black uppercase tracking-tighter" onClick={() => setIsMobileMenuOpen(false)}>{item.name}</a>
            ))}
            <div className="pt-12 border-t border-gray-100">
               <button onClick={() => { setIsMobileMenuOpen(false); setIsGlobalOpen(true); }} className="flex items-center space-x-4 mb-8">
                 <Globe size={20} />
      ...
      <div className={`fixed inset-0 z-[60] transition-opacity duration-500 ${isGlobalOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl transition-transform duration-700 ease-out p-12 flex flex-col ${isGlobalOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex-grow">
            ...
            <div className="space-y-2">
              {languages.map((lang, index) => (
                <button
                  key={lang.code}
                  onClick={() => { onLangChange(lang.code); setTimeout(() => setIsGlobalOpen(false), 300); }}
          ...
          <div className="pt-12 border-t border-gray-100 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 space-y-4">
            <div className="flex justify-between"><span>Currency</span><span className="text-black">KRW (₩)</span></div>
          </div>
        </div>
      </div>
    </>
